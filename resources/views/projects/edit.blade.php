...
<div class="container px-4 py-6 max-w-xl mx-auto">
    <div class="text-2xl font-bold text-var(--primary-dark) mb-4 flex items-center gap-2">
        <span class="text-3xl">📚</span>Modifier projet
    </div>
    ...
    <form action="{{ route('projects.update', $project->id) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ $project->name }}" class="form-input w-full" required>
        </div>

        <div>
            <label class="block font-semibold text-var(--text-dark)">Description</label>
            <textarea name="description" class="form-input w-full" rows="4">{{ $project->description }}</textarea>
        </div>
